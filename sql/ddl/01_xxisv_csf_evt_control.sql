...
CREATE TABLE xxisv.xxisv_csf_evt_control (
  ...
  local_status             VARCHAR2(20) DEFAULT 'PENDING' NOT NULL,
    ...
    -- TIMEOUT     -> não resolveu dentro da janela síncrona; aguardando nova tentativa
  ...
  CONSTRAINT xxisv_csf_evt_control_hdr_uk UNIQUE (event_header_id),
  CONSTRAINT xxisv_csf_evt_control_st_ck CHECK (local_status IN (
    'PENDING', 'SENT', 'IN_PROCESS', 'POLLING',
    'APPROVED', 'CANCELLED', 'ERROR', 'TIMEOUT'
  ))
);

CREATE INDEX xxisv.xxisv_csf_evt_control_st_ix ON xxisv.xxisv_csf_evt_control (local_status);

COMMENT ON TABLE xxisv.xxisv_csf_evt_control IS
