...
CREATE TABLE xxisv.xxisv_csf_evt_http_log (
  ...
  CONSTRAINT xxisv_csf_evt_http_log_ty_ck CHECK (call_type IN ('SEND', 'POLL'))
);

CREATE INDEX xxisv.xxisv_csf_evt_http_log_hdr_ix ON xxisv.xxisv_csf_evt_http_log (event_header_id);

COMMENT ON TABLE xxisv.xxisv_csf_evt_http_log IS
  'Auditoria das chamadas HTTP (UTL_HTTP) do package XXISV_CSF_EVT_COMPLIANCE_PKG para a API da Compliance Fiscal.';
